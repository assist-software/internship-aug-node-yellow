...
exports.create = (req, res) => {
    ...
    EventRequest.create(eventRequest)
        ...
        .catch(err => {
            res.status(500).send({
                message: err.message || "Some error occured while creating the eventMember"
            })
        });
};
...
exports.accept = (req, res) => {
    ...
    EventRequest.findByPk(id)
        .then(requestData => {
            ...
            EventMember.create(requestData.dataValues);
            return EventRequest.destroy({
                where: { id: id }
            });
        ...
        .then(data => {
            res.status(200).send({
                message: "EventRequest accepted successfully!"
            });
        })
        .catch(err => {
            res.status(500).send({
